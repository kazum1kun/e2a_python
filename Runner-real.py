...
def main():
    ...
    progress_bar = tqdm(range(1), desc='Processing test cases...')
    scenario = 'none'
    mapping_file = f'data/mappings/k_missing/{scenario}_fail.txt'
    for act_len in [387]:
        out_folder = f'data/output/real/'
        if not os.path.exists(out_folder):
            os.mkdir(out_folder)

        out_file = f'{out_folder}/{act_len}.json'
        ...
        activity_file = f'data/activities/real/{act_len}.txt'
        event_file = f'data/events/real/{act_len}.txt'

        res = run_e2a(activity_file, event_file, mapping_file, method='mono')
